function [model, metProduction, essentialRxnsForTasks, addedRxnsForTasks, deletedDeadEndRxns, deletedRxnsInINIT, taskReport] = getINITModel2_fast(refModel, tissue, celltype, hpaData, arrayData, metabolomicsData, removeGenes, taskFile, useScoresForTasks, printReport, taskStructure, params, paramsFT)
% getINITModel2
...
%dead-end
if isfield(arrayData,'deletedDeadEndRxns')                                      %LINE ADDED IN FAST VERSION
    deletedDeadEndRxns = arrayData.deletedDeadEndRxns;                          %LINE ADDED IN FAST VERSION
else                                                                            %LINE ADDED IN FAST VERSION
    [~, deletedDeadEndRxns]=simplifyModel(refModel,true,false,true,true,true);
end                                                                             %LINE ADDED IN FAST VERSION
...
if ~isempty(taskStructure)
    if isfield(arrayData,'taskReport') && isfield(arrayData,'essentialRxnMat')   %LINE ADDED IN FAST VERSION
        taskReport = arrayData.taskReport;                                       %LINE ADDED IN FAST VERSION
        essentialRxnMat = arrayData.essentialRxnMat;                             %LINE ADDED IN FAST VERSION
    else                                                                         %LINE ADDED IN FAST VERSION
